...
namespace PrideBot.Events
{
    public class BullshitService
    {
        ...
        public BullshitService(IConfigurationRoot config, DiscordSocketClient client, LoggingService loggingService, ModelRepository repo)
        {
            this.config = config;
            this.client = client;
        ...
        private Task Ready()
        {
            AnnounceLoopAsync(client.GetGyn(config)).GetAwaiter();
            return Task.CompletedTask;
        }

        public async Task AnnounceLoopAsync(SocketGuild guild)
        {
            try
            {
                ...
                while(true)
                {
                    ...
                    Bullshit nextBullshit;
                    do
                    {
                        await Task.Delay(60000);
                        nextBullshit = await GetNextBullshitAsync();
                        if (nextBullshit == null)
                            return;
                    }
                    while (DateTime.Now < nextBullshit.AnnounceTime);

                    await announcementsChannel.SendMessageAsync(nextBullshit.Content);
                    nextBullshit.Announced = true;
                    await UpdateBullshitAsync(nextBullshit);
                }
            }
            catch (Exception e)
            {
        ...
        async Task<Bullshit> GetNextBullshitAsync()
        {
            using var connection = repo.GetDatabaseConnection();
            await connection.OpenAsync();
            return (await repo.GetAllBullshitsAsync(connection))
                .Where(a => !a.Announced && a.AnnounceTime != null && a.AnnounceTime > DateTime.Now.AddMonths(-1))
                .OrderBy(a => a.AnnounceTime)
                .FirstOrDefault();
        }

        async Task UpdateBullshitAsync(Bullshit value)
        {
            using var connection = repo.GetDatabaseConnection();
            await connection.OpenAsync();
            await repo.UpdateBullshitAsync(connection, value);
        }
